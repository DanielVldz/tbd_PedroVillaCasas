...
select avg(cast(amigos as float)) -- se castea a float porque entero no daba decimal
from (
	select ID1,
		count(ID2) as amigos
	from Friend
	group by ID1) hola -- lo último según yo es para darle nombre a la tabla que genera el select dentro del from
...
declare @numeroDeEnamoradosMayor int = (
	-- para obtener el valor de enamorados más grande que hay (2)
	select count(*)
		from Likes
		where ID2 = 1709 -- no sé como hacerlo, es el id de Cassandra :c
);
...
declare @numeroDeAmigosMayor int = max(
	-- para obtener el valor de enamorados más grande que hay (4)
